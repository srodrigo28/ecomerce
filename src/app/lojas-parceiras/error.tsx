...
export default function LojasParceirasError({ reset }: { error: Error; reset: () => void }) {
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-5xl items-center px-4 py-10 sm:px-6 lg:px-8">
      <section className="w-full rounded-[2rem] border border-[var(--border)] bg-[var(--surface)] p-8 text-center shadow-[var(--shadow)] sm:p-10">
        <p className="text-sm font-semibold uppercase tracking-[0.18em] text-amber-600">Falha na vitrine</p>
        <h1 className="mt-3 text-3xl font-semibold theme-heading">Nao foi possivel carregar as lojas parceiras agora.</h1>
        <p className="mt-4 text-sm leading-7 text-[var(--muted)] sm:text-base">
          Isso pode ser temporario. Voce pode tentar novamente ou voltar para a home enquanto seguimos validando o frontend-first.
        </p>
