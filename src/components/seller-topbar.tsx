...
export function SellerTopbar({ store }: { store: StoreSummary }) {
  ...
  useEffect(() => {
    document.documentElement.style.setProperty("--seller-sidebar-width", isCollapsed ? "7rem" : "18rem");
  ...
  return (
    <>
      <aside
        className={`fixed inset-y-0 left-0 z-50 hidden border-r border-white/8 bg-[linear-gradient(180deg,#0f172a_0%,#121b31_52%,#17223a_100%)] text-slate-100 shadow-[0_30px_80px_rgba(15,23,42,0.42)] transition-[width] duration-300 lg:flex ${isCollapsed ? "w-28" : "w-72"}`}
      >
        <div className="flex w-full flex-col">
          <div className={`border-b border-white/8 px-4 py-5 ${isCollapsed ? "flex flex-col items-center gap-3" : "flex items-center gap-3"}`}>
            <div className={`flex ${isCollapsed ? "w-full justify-center" : "min-w-0 flex-1 items-center gap-3"}`}>
              <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-[1.6rem] bg-[linear-gradient(135deg,#22c55e_0%,#14b8a6_52%,#38bdf8_100%)] text-lg font-black text-white shadow-[0_22px_45px_rgba(34,197,94,0.28)]">
              ...
              {!isCollapsed ? (
                <div className="min-w-0">
                  ...
                  <strong className="mt-1 block truncate text-xl font-semibold text-white">{store.name}</strong>
                </div>
              ) : null}
            </div>
            ...
            <button
              ...
              className={`flex shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-white/6 text-slate-100 transition hover:bg-white/10 ${isCollapsed ? "h-11 w-11" : "h-11 w-11"}`}
              ...
              {sellerMenuLinks.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`group flex items-center gap-3 overflow-hidden rounded-[1.35rem] border px-3 py-3.5 transition-all duration-300 ${
                      active
                  ...
                  >
                    <span className={`relative flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl transition ${active ? "bg-emerald-400/16 text-emerald-300" : "bg-white/6 text-slate-200 group-hover:bg-white/10 group-hover:text-white"}`}>
          ...
          <div className="border-t border-white/8 px-4 py-4">
            <Link
              ...
              className={`flex items-center gap-3 rounded-[1.35rem] border border-emerald-400/16 bg-emerald-400/10 px-3 py-3.5 text-emerald-100 transition hover:-translate-y-0.5 hover:bg-emerald-400/16 ${isCollapsed ? "justify-center px-0" : ""}`}
              title={isCollapsed ? "Abrir vitrine" : undefined}
            >
              <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-emerald-400/16 text-emerald-200">
                <FiExternalLink className="h-5 w-5" />
              </span>
              {!isCollapsed ? (
                <span className="min-w-0 flex-1">
                  <span className="block text-sm font-semibold">Abrir vitrine</span>
      ...
      >
        <div className="flex w-full items-center justify-between gap-4 overflow-hidden px-4 py-4 sm:px-6 lg:px-8 2xl:px-10">
          <div className="flex min-w-0 flex-1 items-center gap-3">
            <button
              type="button"
              onClick={() => setMobileMenuOpen(true)}
              className="inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-[1.1rem] border border-[var(--border)] bg-[color:var(--surface)] text-[var(--foreground)] shadow-[var(--shadow-soft)] transition hover:-translate-y-0.5 hover:border-[rgba(var(--accent-rgb),0.22)] lg:hidden"
            ...
            <div className="min-w-0">
              ...
              <div className="mt-1 flex min-w-0 items-center gap-3">
                <div className="hidden h-12 w-12 shrink-0 items-center justify-center rounded-[1.2rem] bg-[linear-gradient(135deg,rgba(var(--accent-rgb),0.18),rgba(var(--accent-rgb),0.08))] text-[var(--accent)] shadow-[inset_0_1px_0_rgba(255,255,255,0.5)] sm:flex">
                  <FiGrid className="h-5 w-5" />
                </div>
                <div className="min-w-0">
                  <strong className="block truncate text-lg font-semibold theme-heading sm:text-xl">{currentSection.title}</strong>
                  <span className="block truncate text-sm text-[var(--muted)]">{currentSection.subtitle}</span>
                </div>
              </div>
            </div>
          </div>

          <div className="flex shrink-0 items-center gap-2 pr-[100px] sm:gap-3">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-3 rounded-[1.35rem] border border-[var(--border)] bg-[color:var(--surface)] px-3 py-2.5 shadow-[var(--shadow-soft)] transition hover:-translate-y-0.5 hover:border-[rgba(var(--accent-rgb),0.22)]"
              >
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-[1rem] bg-[linear-gradient(135deg,#0f172a_0%,#1e293b_52%,rgba(var(--accent-rgb),0.85)_100%)] text-sm font-semibold text-white shadow-[0_16px_34px_rgba(15,23,42,0.18)]">
                  {(store.name || "L").slice(0, 1).toUpperCase()}
                </span>
                <span className="hidden min-w-0 text-left sm:block">
                  <span className="block max-w-[180px] truncate text-sm font-semibold theme-heading">{store.name}</span>
                  <span className="block max-w-[180px] truncate text-xs text-[var(--muted)]">Minha loja conectada</span>
                </span>
              </button>

              {open ? (
                <div className="absolute right-0 mt-3 w-[min(92vw,340px)] rounded-[1.7rem] border border-[var(--border)] bg-[color:var(--surface-strong)]/96 p-3 shadow-[var(--shadow)] backdrop-blur-2xl">
