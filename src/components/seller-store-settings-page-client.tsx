...
export function SellerStoreSettingsPageClient({ store }: { store: StoreSummary }) {
      ...
      <section className="rounded-[2rem] border border-[var(--border)] bg-[var(--surface)] p-5 shadow-[var(--shadow)] sm:p-8 lg:p-10">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Link href={`/lojas/${store.slug}`} className="rounded-full theme-border-button px-5 py-3 text-sm font-semibold transition">
              Abrir vitrine
            </Link>
          </div>
        </div>
